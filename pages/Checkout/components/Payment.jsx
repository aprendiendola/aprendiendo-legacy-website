...
class Payment extends Component {
  ...
  componentDidMount() {
    const { orderDetails, totalPrice, clearCouponErrorMessage } = this.props;

    clearCouponErrorMessage();
  ...
  getDiscountedPrice = () => {
    const { coupon, totalPrice } = this.props;

    let discountedAmount = 0;

    if (coupon.percent_off) {
      discountedAmount = applyPercentDiscount(totalPrice, coupon.percent_off);
    } else {
      discountedAmount = applyAmountDiscount(totalPrice, coupon.amount_off);
    }
    ...
    if (response[0]) {
      ...
      if (migrate) {
        ...
        facebookPixel.orderCompleted({
          ...
          num_items: 1,
          value: orderDetails[0].amount / 100,
          currency: service.getCountry().currency
        });
        ...
        ReactGA.plugin.execute("ecommerce", "addItem", {
          ...
          price: orderDetails[0].amount / 100,
          category: "Package",
          quantity: "1"
        });
        ReactGA.plugin.execute("ecommerce", "addTransaction", {
          ...
          sold_price: orderDetails[0].amount / 100,
          shipping: "0",
          tax: "0"
        });
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div>
              <span className="total-price">
                ...
                {coupon ? this.getDiscountedPrice() : totalPrice}
              </span>
              <span
                className="total-price-before"
                style={{ visibility: !coupon && "hidden" }}
              >
                {service.getCountry().currencySymbol}
                {totalPrice}
              </span>
          ...
          {service.getCountry().countryCode.toUpperCase() === "PE" ? (
            <Fragment>
              <Accordion
                ...
                content={
                  <CashContent
                    processCashPayment={dni => this.processCashPayment(dni)}
                    loading={isCashLoading}
                  />
                }
                type="checkbox"
                isExpanded={currentMethod === PAYMENT_METHODS.CASH}
              />
              <Accordion
                heading={
                  <Heading
...
const mapDispatchToProps = {
  ...
  saveCoupon: coupon => setCoupon(coupon),
