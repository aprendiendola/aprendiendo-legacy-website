...
class Payment extends Component {
  ...
  afterSubmit = response => {
    ...
    if (response.status === 201) {
      ...
      if (response.data) {
        errorMessage = ERROR_MESSAGES[response.data.message] ? ERROR_MESSAGES[response.data.message] : errorMessage;
      }
      ...
      coupon && (
        <div style={{ marginTop: '10px' }}>
          <Alert fontWeight={600}>
            Bien hecho, ¡Se ha aplicado el cupón con éxito para tu primer pago de suscripción! A partir del
            segundo pago se cobrará el precio normal.
          </Alert>
        </div>
      )
    );
